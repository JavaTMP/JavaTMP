...
JavaTMP templates layouts are based on fixed-fluid two columns bootstrap 4 layout
which you can find separate information and demos in the following pages:
...
*   [off canvas sliding fixed sidebar in bootstrap 4 using css transition demo](http://java.javatmp.com/pages/static/custom-pages/two-columns-layout/off-canvas-sliding-fixed-sidebar-in-bootstrap-4-using-css-transition "off canvas sliding fixed sidebar in bootstrap 4 using css transition demo")
...
*   [on canvas sliding hidden fixed sidebar in bootstrap 4 on mouse hover demo](http://java.javatmp.com/pages/static/custom-pages/two-columns-layout/on-canvas-sliding-hidden-fixed-sidebar-in-bootstrap-4-on-mouse-hover "on canvas sliding hidden fixed sidebar in bootstrap 4 on mouse hover demo")
...
Also you should make sure any fixed element with explicit height are style by `overflow-y: scroll;`.
...
*   `.main-sidebar-menu` div which provides collapsible sidebar links using `metismenu` jQuery plugin.
...
`.main-sidebar-menu` `ul` is a container for main sidebar collapsible menu items. The menu is initialized and activated in
...
*   Optionally, you can remove or update `.main-sidebar-menu.metismenu` class in sidebar and use another class too in `index.js` file
    for handling event `click` on any link inside `.main-sidebar-menu.metismenu`. for more information about
    custom styling of sidebar menu see sass file `.\web\assets\src\sass\plugins\custom-metisMenu.scss`.
    ```
    ...
    // keep in mind if you don't add `.metismenu` class to element initialized by metisMenu jQuery plugin. it will add explicitly
    // when the plugin initialized.
    javatmp.settings.mainSidebarMenuClass = ".main-sidebar-menu";
    ...
