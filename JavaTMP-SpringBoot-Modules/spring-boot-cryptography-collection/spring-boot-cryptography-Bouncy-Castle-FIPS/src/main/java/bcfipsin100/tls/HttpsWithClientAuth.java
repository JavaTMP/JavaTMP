...
public class HttpsWithClientAuth {
    ...
    private static KeyStore rebuildStore(String storeType, char[] storePassword,
                                         byte[] encoding)
    ...
    public static void main(String[] args)
        ...
        X509Certificate caCert = Cert.makeV1Certificate(caKeyPair.getPrivate(),
                caKeyPair.getPublic());

        KeyStore keyStore = rebuildStore("jks", storePass, KeyStr
                .storePrivateKey(storePass, keyPass, caKeyPair.getPrivate(),
                        new X509Certificate[]{caCert}));

        Util.runClientAndServer(
                new HttpsAuthServer(keyStore, keyPass, keyStore),
                new HttpsAuthClient(keyStore, keyStore, keyPass));
    }
    ...
    public static class HttpsAuthClient
        ...
        public Object call()
                throws Exception {
            TrustManagerFactory trustMgrFact = TrustManagerFactory
                    .getInstance("SunX509");

            trustMgrFact.init(trustStore);

            KeyManagerFactory keyMgrFact = KeyManagerFactory
                    .getInstance("SunX509");
        ...
        private class LocalHostVerifier
            ...
            public boolean verify(String hostName, SSLSession session) {
                try {
                    X500Principal hostID = (X500Principal) session
                            .getPeerPrincipal();

                    return hostName.equals("localhost") && hostID.getName()
                            .equals("CN=Issuer CA");
                } catch (Exception e) {
                    return false;
                }
    ...
    public static class HttpsAuthServer
        ...
        HttpsAuthServer(KeyStore serverStore, char[] keyPass,
                        KeyStore trustStore) {
        ...
        public Object call()
            ...
            SSLServerSocketFactory fact = serverContext
