...
public class Aes {
    public static SecretKey generateKey() throws GeneralSecurityException {
        ...
        keyGenerator.init(256); // 256 / 8 = 32 bytes.
    ...
    public static byte[] ccmDecrypt(SecretKey key,
                                    AlgorithmParameters ccmParameters,
                                    byte[] cipherText)
            throws GeneralSecurityException {
        Cipher cipher = Cipher.getInstance("AES/CCM/NoPadding", "BCFIPS");
    ...
    public static byte[] aeadDecrypt(SecretKey key,
                                     AlgorithmParameters ccmParameters,
    ...
    public static void main(String[] args)
        ...
        System.err.println(Arrays.areEqual(
                ExValues.SampleInput,
                ecbDecrypt(secKey, ecbEncrypt(secKey, ExValues.SampleInput))));

        byte[][] cbcOutput = cbcEncrypt(secKey, ExValues.SampleInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                cbcDecrypt(secKey, cbcOutput[0], cbcOutput[1])));

        byte[][] cfbOutput = cfbEncrypt(secKey, ExValues.SampleInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                cfbDecrypt(secKey, cfbOutput[0], cfbOutput[1])));

        byte[][] ctrOutput = ctrEncrypt(secKey, ExValues.SampleInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                ctrDecrypt(secKey, ctrOutput[0], ctrOutput[1])));

        Object[] gcmOutput = gcmEncrypt(secKey, ExValues.SampleInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                gcmDecrypt(secKey, (AlgorithmParameters) gcmOutput[0],
                        (byte[]) gcmOutput[1])));

        Object[] ccmOutput = ccmEncrypt(secKey, ExValues.SampleInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                ccmDecrypt(secKey, (AlgorithmParameters) ccmOutput[0],
                        (byte[]) ccmOutput[1])));

        Object[] aeadOutput = aeadEncrypt(secKey, ExValues.SampleInput,
                ExValues.SampleTwoBlockInput);
        System.err.println(Arrays.areEqual(ExValues.SampleInput,
                aeadDecrypt(secKey, (AlgorithmParameters) aeadOutput[0],
                        (byte[]) aeadOutput[1], ExValues.SampleTwoBlockInput)));

        byte[][] ctsOutput = ctsEncrypt(secKey, ExValues.SampleTwoBlockInput);
        System.err.println(Arrays.areEqual(ExValues.SampleTwoBlockInput,
                ctsDecrypt(secKey, ctsOutput[0], ctsOutput[1])));
        ...
        System.err.println(
                Hex.toHexString(generateMacCMAC(secKey, ExValues.SampleInput)));
        System.err.println(
                Hex.toHexString(generateMacGMAC(secKey, ExValues.SampleInput)));
        System.err.println(
                Hex.toHexString(generateMacCCM(secKey, ExValues.SampleInput)));

        System.err.println(Arrays.areEqual(ExValues.SampleAesKey.getEncoded(),
                unwrapKey(secKey, wrapKey(secKey, ExValues.SampleAesKey))
                        .getEncoded()));
        System.err.println(Arrays.areEqual(ExValues.SampleHMacKey.getEncoded(),
                unwrapKeyWithPadding(secKey,
                        wrapKeyWithPadding(secKey, ExValues.SampleHMacKey))
                        .getEncoded()));
    }
}
