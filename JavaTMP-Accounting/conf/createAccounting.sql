...
SUM(entryAmount) OVER(PARTITION BY moduleId ORDER BY entryDate, id
) AS moduleIdBalance,
SUM(entryAmount) OVER(PARTITION BY moduleId,moduleTypeId,moduleRefId ORDER BY entryDate, id
) AS entityBalance
from (
